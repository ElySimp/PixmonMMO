...
async function fixUserStatsComplete() {
  try {
    ...
    // Re-insert only the latest records for each user with the correct columns
    let reinserted = 0;
    ...
    for (const record of latestRecords) {
      ...
      const formattedQuestPointCooldown = quest_point_cooldown ? new Date(quest_point_cooldown).toISOString().slice(0, 19).replace('T', ' ') : null;
      const formattedLastReset = last_reset ? new Date(last_reset).toISOString().slice(0, 19).replace('T', ' ') : null;
